...
export function Sidebar({ items }: { items: NavItem[] }) {
  ...
  return (
    <aside className="flex h-screen w-full flex-col bg-white dark:bg-neutral-900 border-r border-neutral-200/80 dark:border-neutral-800/60 transition-colors duration-200 overflow-hidden">
      ...
      {/* Footnote Core Footer Profiler Segment */}
      <div className="shrink-0 border-t border-neutral-100 dark:border-neutral-800/80 bg-white dark:bg-neutral-900 p-4 mt-auto">
        {/* Profile Card Integration Container */}
        <div className="flex items-center gap-3 rounded-xl border border-neutral-200/80 dark:border-neutral-800 p-3 bg-neutral-50/50 dark:bg-neutral-950/40">
          ...
          {/* Embedded Inline Sign Out Action Button */}
          <button
            onClick={logout}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-neutral-200/60 dark:border-neutral-800 text-neutral-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50/80 dark:hover:bg-red-950/30 transition-all active:scale-95 group"
